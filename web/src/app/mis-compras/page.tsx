...
export default async function MisComprasPage() {
  ...
  const ops = all.filter(
    (op) => resolveRoleForOperation(db, op, account.account_id, account.email_canonical) === "BUYER",
  );
    ...
    <AppShell account={account}>
      ...
      {ops.length === 0 ? (
        <div className="empty card">
          <p>No participaste en compras todavía.</p>
          <p className="text-secondary">Cuando alguien te envíe una solicitud de pago, va a aparecer acá.</p>
        </div>
      ) : (
